...
class Database:
    ...
    def update_database_entry(self, old_entry, new_entry):
        with sqlite3.connect(self.path) as connection:
            ...
            update_query = f"UPDATE {self.name} SET " + ", ".join([f'{col}=?' for col in self.camel_columns]) + " WHERE " + " AND ".join([(f'{col}=?' if old_entry[self.columns[i]] else f'{col} IS NULL') for i, col in enumerate(self.camel_columns)])
    ...
    def new_entry(self, new_entry):
        ...
        with sqlite3.connect(self.path) as connection:
            ...
            cursor.execute(
                insertion_query,
                [new_entry[col] for col in self.columns]
            )
